function setGlobalFunctions(form, customHTML) {
	...
	customHTML.append(
	  "function getWKNumProces(){ return " + getValue("WKNumProces") + "; }"
	);
	customHTML.append(
	  "function getWKNumState(){ return " + getValue("WKNumState") + "; }"
	);
	customHTML.append(
	  "function getOrganizationId(){ return " +
		getValue("WKIdentityCompany") +
		"; }"
	);
	customHTML.append(
	  "function getTenantCode(){ return " + getValue("WKCompany") + "; }"
	);
	customHTML.append(
	  'function getUserCode(){ return "' +
		fluigAPI.getUserService().getCurrent().getCode().trim() +
		'"; }'
	);
	customHTML.append(
	  'function getUserLogin(){ return "' +
		fluigAPI.getUserService().getCurrent().getLogin().trim() +
		'"; }'
	);
	customHTML.append(
	  'function getUserEmail(){ return "' +
		fluigAPI.getUserService().getCurrent().getEmail().trim() +
		'"; }'
	);
	customHTML.append(
	  'function getUserName(){ return "' +
		fluigAPI.getUserService().getCurrent().getFullName().trim() +
		'"; }'
	);
	customHTML.append(
	  "function getWKMobile(){ return " +
		(form.getMobile() != null && form.getMobile() ? true : false) +
		"; }"
	);
	customHTML.append(
	  "function getEditMode(){ return " +
		(form.getFormMode() == "VIEW" ? false : true) +
		"; }"
	);
	customHTML.append(
  ...
  function enableContainer(form, customHTML, object, enable) {
	...
	customHTML.append('<script type="text/javascript">');
	customHTML.append(
	  "setTimeout(function() { enableContainer(" +
		object +
		", " +
		enable +
		"); }, 1);"
  ...
  function enableField(form, customHTML, object, enable) {
	customHTML.append('<script type="text/javascript">');
	customHTML.append(
	  'setTimeout(function() { enableField($("#' +
		object +
		'"), ' +
		enable +
		"); }, 1);"
  ...
  function setFieldCalendar(form, customHTML, object, enable) {
	customHTML.append('<script type="text/javascript">');
	customHTML.append(
	  'setTimeout(function() { FLUIGC.calendar("#' +
		object +
		'", {useCurrent: ' +
		enable +
		"}); }, 1);"
  ...
  function setFieldRichEditor(form, customHTML, object, readOnly, settings) {
	...
	customHTML.append("setTimeout(function() {");
	customHTML.append(
	  "var " + object + ' = FLUIGC.richeditor("' + object + '", {'
	);
	...
	customHTML.append(
	  "" + object + ".editor.config.readOnly = " + readOnly + ";"
	);
	customHTML.append("" + object + '.on("change", function(e) {');
	customHTML.append(
	  "	var body = " + object + ".editor.document.getBody().$.innerHTML;"
	);
  ...
  function panelShow(form, customHTML, object, enable) {
	...
	if (enable == true) {
	  customHTML.append(
		'setTimeout(function() { $("#' + object + '").show(); }, 1);'
	  );
	} else {
	  customHTML.append(
		'setTimeout(function() { $("#' + object + '").hide(); }, 1);'
	  );
	}
  ...
  function collapseShow(form, customHTML, object, enable) {
	...
	if (enable == true) {
	  customHTML.append(
		'setTimeout(function() { $("#' + object + '").collapse("show"); }, 1);'
	  );
	} else {
	  customHTML.append(
		'setTimeout(function() { $("#' + object + '").collapse("hide"); }, 1);'
	  );
	}
