...
$(document).ready(function (form, customHTML) {
...
//Busca usuarios dos grupos, subgrupos e subpapeis
//console.log(getUsuariosViaGroupUserRole("C001 - Solicitantes"));
function getUsuariosViaGroupUserRole(grupoPai) {
    var usuarios = [];

    // 1. Consulta a tabela fdn_groupuserrole via DS_TABLE_FLUIG
    ...
    var ds = DatasetFactory.getDataset("DS_TABLE_FLUIG", null, [c1], null);
    
    for (var i = 0; i < ds.values.length; i++) {
        var row = ds.values[i];

        // 1. LOGIN → dataset colleague
        if (row.LOGIN && row.LOGIN.trim() !== "") {
            var cLogin = DatasetFactory.createConstraint("login", row.LOGIN.trim(), row.LOGIN.trim(), ConstraintType.MUST);
            var dsColleague = DatasetFactory.getDataset("colleague", null, [cLogin], null);
            for (var j = 0; j < dsColleague.values.length; j++) {
                var user = dsColleague.values[j]["colleaguePK.colleagueId"];
                if (usuarios.indexOf(user) === -1) {
                    usuarios.push(buscaUsuario(user, "colleague"));
                }
            }
        }

        // 2. CHILD_GROUPCODE → dataset colleagueGroup
        if (row.CHILD_GROUPCODE && row.CHILD_GROUPCODE.trim() !== "") {
            var cGroup = DatasetFactory.createConstraint("groupId", row.CHILD_GROUPCODE.trim(), row.CHILD_GROUPCODE.trim(), ConstraintType.MUST);
            var dsGroup = DatasetFactory.getDataset("colleagueGroup", null, [cGroup], null);
            for (var k = 0; k < dsGroup.values.length; k++) {
                var user = dsGroup.values[k]["colleagueGroupPK.colleagueId"];
                if (usuarios.indexOf(user) === -1) {
                    usuarios.push(buscaUsuario(user, "group"));
                }
            }
        }
    
        // 3. ROLE_CODE → dataset workflowColleagueRole
        if (row.ROLE_CODE && row.ROLE_CODE.trim() !== "") {
            var cRole = DatasetFactory.createConstraint("roleId", row.ROLE_CODE.trim(), row.ROLE_CODE.trim(), ConstraintType.MUST);
            var dsRole = DatasetFactory.getDataset("workflowColleagueRole", null, [cRole], null);
            for (var m = 0; m < dsRole.values.length; m++) {
                var user = dsRole.values[m]["workflowColleagueRolePK.colleagueId"];
                if (usuarios.indexOf(user) === -1) {
                    usuarios.push(buscaUsuario(user, "role"));
                }
            }
        }
...
function buscaUsuario(id, tipo){
    console.log(id + " - " + tipo )
    var cColleague = DatasetFactory.createConstraint("colleaguePK.colleagueId", id, id, ConstraintType.MUST);
    var dsColleague = DatasetFactory.getDataset("colleague", null, [cColleague], null);

    
    if(dsColleague.values.length > 0) return dsColleague.values[0].colleagueName; //aqui vc retorna a coluna que quiser 
    else return "";
